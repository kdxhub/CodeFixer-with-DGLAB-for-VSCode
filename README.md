...
* 当前「问题」状态栏提示的总错误数决定强度，转化比例可自行设置
* 终端命令执行出错，也会按比例转为强度。该强度还可设置时长，甚至是永久
* A/B波形图上的按钮分别是 `-5` / `-1` / `-` / `+1` / `+5` ，可以设置基准值。
* 新建文件，键入，鼠标点击，保存文件，添加断点，开始调试……

上述强度叠加而不相互影响，但不会超过APP内设置的强度上限。
...
除此之外，APP 上方形按钮被点击也会触发该保护措施。

# 推荐实例（玩法）
## 受着
*爱思*负责写代码，*爱慕*负责被固定挨电。想少被电？求主人少写点BUG吧！

## 不写代码就无法停止被��
将*爱慕*拷在电脑前，把基础强度设很高，之后将配置里面的`当「键入字符」`等改成负值，这样只有 Ta 不停写代码强度才会降低。

## 高Ping战士
将配置项里面的`更新间隔`设置很高的数值，那样强度变化会在很长一段时间后才会下发。

## 禁止写代码
将*爱慕*拷在电脑前，将每个操作的强度设置得很高，电脑锁定 VSCode 无法运行其它软件进行放置普雷。
